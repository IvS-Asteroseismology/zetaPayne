...
import zetapayne.common_functions as cf
...
class UncertFit:
    ...
    def run(self, wave, flux, flux_err, p0=None):
        return self._run_3(wave, flux, flux_err, p0)
    ...
    def _run_3(self, wave, flux, flux_err, p0 = None):
        ...
        for pn in grid_params:
            if len(cf.GSSP_steps[pn]) == 1:
                step = cf.GSSP_steps[pn][0]
            else:
                step = cf.GSSP_steps[pn][1]
            steps[pn] = step
    ...
    def _run_2(self, wave, flux, flux_err, p0=None):
        wave_start = min(wave)
        wave_end = max(wave)
        ndegree = 4 * self.resol * (wave_end - wave_start)/(wave_end + wave_start)
        CHI2_C = 1.0 + np.sqrt(2.0 / ndegree)
